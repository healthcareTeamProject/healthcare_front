...
function SignUpFirst({ onNext, name, setName, id, setId }: SignUpFirstProps) {
    const [idMessage, setIdMessage] = useState<string>('');
    const [idMessageError, setIdMessageError] = useState<boolean>(false);
    const [isCheckedId, setCheckedId] = useState<boolean>(false);
    const isComplete = name && id && isCheckedId;
...
function SignUpSecond({ onPrevious, height, setHeight }: SignUpSecondProps) {
    const [heightMessageError, setHeightMessageError] = useState<boolean>(false);
    ...
    const onHeightChangeHandler = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        if (pattern.test(value)) {
            setHeight(value);
            setHeightMessageError(false);
        } else {
            setHeightMessageError(true);
        }
    };
        ...
        <div id='su-wrapper'>
            ...
            <div id='su-main'>
                <div className='sign-up-contain'>
                    <div className='sign-up-title'>회원가입</div>
                    {signUpPage ? 
                        (<SignUpFirst onNext={onSignUpPageChangeHandler} name={name} setName={setName} id={id} setId={setId} />) : 
                        (<SignUpSecond onPrevious={onSignUpPageChangeHandler} height={height} setHeight={setHeight} />)
                    }
                </div>
            </div>
        </div>
